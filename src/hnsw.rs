...
/// This unit structure provides the type to instantiate Hnsw with,
...
/// This type is an identifier of each data vector (given by the client).
...
pub struct Hnsw<T: Clone + Send + Sync, D: Distance<T>> {
    ...
    /// max layer , recall that rust is in 0..maxlevel right bound excluded
    pub(crate) max_layer: usize,
